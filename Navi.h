//-----------------------------------------------------------
// Ayden Jay Soliz
// Navi compiler "global" definitions and the common classes
//    NaviEXCEPTION, LISTER, READER, CODE, and IDENTIFIERTABLE
//
...
enum DATATYPE { NOTYPE,INTEGERTYPE,BOOLEANTYPE,FLOATTYPE };
...
public:
   //-----------------------------------------------------------
   NAVIEXCEPTION(const char description[])
   //-----------------------------------------------------------
   {
...
//-----------------------------------------------------------
{
   ...
   strcat(this->sourceFileName,".navi");
   strcpy(fullFileName,sourceFileName);
   strcat(fullFileName,".list");
   LIST.open(fullFileName,ios::out);
   if ( !LIST.is_open() ) throw( NAVIEXCEPTION("Unable to open list file") );
   ListTopOfPageHeader();
}
...
//-----------------------------------------------------------
{
   ...
   strcpy(fullFileName,sourceFileName);
   strcat(fullFileName,".navi");
   SOURCE.open(fullFileName,ios::in);
   if ( !SOURCE.is_open() ) throw( NAVIEXCEPTION("Unable to open source file") );
...
// index in [ 0,LOOKAHEAD ] where index = 0 means last GetNextCharacter() returned
   if ( (0 <= index) && (index <= LOOKAHEAD) )
      ...
      throw( NAVIEXCEPTION("GetLookAheadCharacter() index out-of-range") );
}
...
//-----------------------------------------------------------
{
   if ( numberCallbacks <= CALLBACKSALLOWED )
      ...
      throw( NAVIEXCEPTION("Too many callback functions") );
}
...
*/
   if ( identifiers > capacity )
      throw( NAVIEXCEPTION("Identifier table capacity exceeded") );
   else
   {
      identifiers++;
...
//--------------------------------------------------
{
   ...
   if ( !STM.is_open() ) throw( NAVIEXCEPTION("Unable to open code file") );
}
...
//--------------------------------------------------
{
   ...
   EmitUnformattedLine("");
   EmitUnformattedLine(";------------------------------------------------------------");
   EmitUnformattedLine("; Heap space for dynamic memory allocation (to support future Navi syntax)");
   EmitUnformattedLine(";------------------------------------------------------------");
